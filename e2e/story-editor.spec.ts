...
test.describe("Story Editor", () => {
  ...
  test("should show image of the day", async ({ page }) => {
    ...
    const image = page.getByRole("img", { name: "Image of the day" });
    await expect(image).toBeVisible({ timeout: 10000 });

    // Verify image has a valid src
    const src = await image.getAttribute("src");
    expect(src).toBeTruthy();
    expect(src).toContain("example.com/mock-image.jpg");
  });
  ...
  test("should allow writing and saving a story", async ({ page }) => {
    ...
    const savedStory = await page.evaluate(() =>
      window.localStorage.getItem("todayStory")
    );
  ...
  test("should maintain story state across browser tabs", async ({
    context,
  }) => {
